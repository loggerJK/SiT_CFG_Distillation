...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in tqdm(loader, desc=f"Epoch {epoch + 1}/{args.epochs}", disable=rank != 0, leave=False, total=len(loader)):
            ...
            g = 4.0
            model_kwargs = dict(y=y, g=torch.Tensor([g] * n).to(device))  # Guidance scale
            model_cfg_kwargs = dict(y=y, cfg_scale=torch.Tensor([g] * n).to(device))  # Guidance scale
