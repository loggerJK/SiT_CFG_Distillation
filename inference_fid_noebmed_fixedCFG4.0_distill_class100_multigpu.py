...
ckpt_path = '/media/dataset2/jiwon/sit_train_cfg_distill/results_no_embed_cfg4.0_bf16/SiT_noembed_fixedCFG4.0_bf16_lr1e-4/epoch0.pt'
model.eval()
...
# ------------ 저장 폴더 생성 ------------ #
if rank == 0:
    for mode in ['distill_fixed4.0_class100']:
...
for global_idx in tqdm(image_indices_this_rank, desc=f"Generating on rank {rank}"):
    ...
    path_original = os.path.join(output_root, 'distill_fixed4.0_class100', f"img_{class_label:03}_{img_idx:03}.png")
    ...
    save_single_image(decoded_origin, os.path.join(output_root, 'distill_fixed4.0_class100'), class_label, img_idx)
