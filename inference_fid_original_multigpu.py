...
ckpt_path = '/media/dataset2/jiwon/sit_train_cfg_distill/SiT-XL-2-256.pt'
model.eval()
...
for global_idx in tqdm(image_indices_this_rank, desc=f"Generating on rank {rank}"):
    ...
    path_original = os.path.join(output_root, 'original_reeval', f"img_{class_label:03}_{img_idx:03}.png")
    ...
    save_single_image(decoded_origin, os.path.join(output_root, 'original_reeval'), class_label, img_idx)
