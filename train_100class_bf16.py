...
def main(args):
    ...
    weight_dtype = torch.bfloat16 # if torch.cuda.is_bf16_supported() else torch.float32
    ...
    for name, param in model.named_parameters():
        ...
        # if param.requires_grad == False:
            # raise ValueError(f"Parameter {name} is not trainable!")
    ...
    vae = AutoencoderKL.from_pretrained(f"stabilityai/sd-vae-ft-{args.vae}").to(device).to(weight_dtype)
    vae.enable_slicing()
    ...
    n = ys.size(0)
    zs = torch.randn(n, 4, latent_size, latent_size, device=device, dtype=weight_dtype)
    ...
    if use_cfg:
        ...
        sample_model_kwargs = dict(y=ys, g=torch.Tensor([args.cfg_scale] * n).to(device, dtype=weight_dtype))  # Guidance scale
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in tqdm(loader, desc=f"Epoch {epoch + 1}/{args.epochs}", disable=rank != 0, leave=False, total=len(loader)):
            ...
            # Sample random guidance scale for training: uniform 1 to 9
            g = torch.rand(n, device=device, dtype=weight_dtype) * (9.0 - 1.0) + 1.0
            ...
            with torch.no_grad():
                if train_steps % args.sample_every == 0 and train_steps > 0:
                    ...
                    out_samples = torch.zeros((args.global_batch_size, 3, args.image_size, args.image_size), device=device, dtype=weight_dtype)
                    ...
                    sample_fn = transport_sampler.sample_ode() # default to ode sampling
                    g = torch.Tensor([1.0] * n).to(device, dtype=weight_dtype)  # Baseline guidance scale
